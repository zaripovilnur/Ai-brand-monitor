...
# hostname -I показывает все адреса подряд, включая внутренние (192.168.*, 10.*),
# по которым сервер снаружи не открыть. Спрашиваем настоящий внешний адрес.
addr=$(curl -fsS --max-time 5 https://api.ipify.org 2>/dev/null || true)
if [ -z "$addr" ]; then
  addr=$(hostname -I | tr ' ' '\n' \
    | grep -Ev '^(10\.|127\.|169\.254\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.)' \
    | head -1)
fi
addr_note=""
if [ -z "$addr" ]; then
  addr=$(hostname -I | awk '{print $1}')
  addr_note="  Внимание: похоже, это внутренний адрес — снаружи он не откроется.
  Публичный адрес сервера смотрите в панели хостинга.
"
fi
